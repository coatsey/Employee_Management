...
function mainMenu() {
  const menuPrompt = new InquirerFunctions(
    inquirerTypes[2],
    "menuChoice",
    questions.mainMenuPrompt,
    commandMenuChoices
  );
...
function viewAllEmpDep(depNamesArray) {
  const departmentNamePrompt = new InquirerFunctions(
    inquirerTypes[2],
    "department_Name",
    questions.viewAllEmpByDep,
    depNamesArray
  );
...
function viewAllEmpRole(compRoles, actionChoice) {
  const rolePrompt = new InquirerFunctions(
    inquirerTypes[2],
    "role_Title",
    questions.viewAllEmpByRole,
    compRoles
  );
...
function EmpInfoPrompts(compRoles, actionChoice) {
  ...
  connection.query(query, function (err, res) {
    ...
    const first_name = new InquirerFunctions(
      inquirerTypes[0],
      "first_name",
      questions.addEmployee1
    );
    const last_name = new InquirerFunctions(
      inquirerTypes[0],
      "last_name",
      questions.addEmployee2
    );
    const emp_role = new InquirerFunctions(
      inquirerTypes[2],
      "employee_role",
      questions.addEmployee3,
      compRoles
    );
    const emp_manager = new InquirerFunctions(
      inquirerTypes[2],
      "employee_manager",
      questions.addEmployee4,
      managerNamesArr
    );
...
function addEmp(emp_info, managerObjArr) {
  ...
  connection.query(
    queryRoleIdFromTitle,
    emp_info.employee_role,
    function (err, res) {
      if (err) {
        throw err;
      }
      ...
      for (let manager of managerObjArr) {
        if (
          manager.firstName == empManagerFirstName &&
          manager.lastName === empManagerLastName
        ) {
          empManagerID = manager.ID;
        }
      }

      const queryInsertEmpInfo =
        "INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES (?, ?, ?, ?)";
      connection.query(
        queryInsertEmpInfo,
        [empFirstName, empLastName, empRoleId, empManagerID],
        function (err, res) {
          if (err) {
            throw err;
          }
  ...
  connection.query(
    ...
    function (err, res) {
      if (res.length > 1) {
        ...
        const which_employee_to_Delete = new InquirerFunctions(
          inquirerTypes[2],
          "employee_delete",
          questions.deleteEmployee1,
          multipleName
        );
...
function deleteEmp(firstName, lastName, employeeID) {
  ...
  const confirmDelete = new InquirerFunctions(
    inquirerTypes[2],
    "confirm_choice",
...
function updateEmpRole(employeeID, RolesArray) {
  ...
  const empNewRole = new InquirerFunctions(
    inquirerTypes[2],
    "employee_role",
  ...
  inquirer.prompt([empNewRole.ask()]).then((chosenRole) => {
    connection.query(
      ...
      function (err, res) {
        ...
        const updateEmpRoleId = new SQLquery(queryUpdateRoleId, [
          res[0].id,
          employeeID,
        ]);
...
function updateEmpManager(employeeID, managerObjectArray) {
  ...
  connection.query(queryCurrentManager, employeeID, function (err, res) {
    ...
    const newManagerChoice = new InquirerFunctions(
      inquirerTypes[2],
      "new_Manager",
...
function addRole() {
  ...
  connection.query(queryDeps, function (err, res) {
    ...
    const whatRole = new InquirerFunctions(
      inquirerTypes[0],
      "role_to_add",
      questions.newRole
    );
    const whatSalary = new InquirerFunctions(
      inquirerTypes[0],
      "role_salary",
      questions.salary
    );
    const whatdepartment = new InquirerFunctions(
      inquirerTypes[2],
      "department",
          ...
          connection.query(
            ...
            function (err, res) {
              ...
              const addRolequery = `INSERT INTO role (role.title, role.salary, role.department_id)
                                    VALUES ( (?), (?), (?));`;
              const addRole = new SQLquery(addRolequery, [
                userChoices.role_to_add,
                userChoices.role_salary,
                res[0].id,
              ]);
        ...
        if (res.length > 1) {
          ...
          connection.query(
            ...
            function (err, res) {
              ...
              const whichDeparment = new InquirerFunctions(
                inquirerTypes[2],
                "department_to_delete_Role_From",
                questions.departmentDeleteRole,
                departmentsWithRoleArr
              );
              ...
              inquirer.prompt([whichDeparment.ask()]).then((userChoice) => {
                ...
                const departmentName_ID_Arr = res.filter((department) => {
                  if (
                    department.name == userChoice.department_to_delete_Role_From
                  ) {
                    return true;
                  }
                });
                ...
                deleteRoleQuery2 =
                  "DELETE FROM role WHERE role.title = (?) AND role.department_id = (?)";
                const deleteInstance2 = new SQLquery(deleteRoleQuery2, [
                  roleDeleteTitle,
                  departmentName_ID_Arr[0].department_id,
                ]);
...
function addDep(depNameArr) {
  const whatDep = new InquirerFunctions(
    inquirerTypes[0],
    "dep_to_add",
    questions.newDep
  );
